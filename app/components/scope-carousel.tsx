...
const ScopeCarousel = ({ onScopeChange }: ScopeCarouselProps) => {
  const { activeIndex, isTransitioning, handleScopeHover, handleMouseLeave } =
    useScopeCarousel(scopes.length);
          ...
          >
            <Tooltip open={index === activeIndex}>
              <TooltipTrigger asChild>
                <figure
                  className={cn(
                    'relative w-20 aspect-square rounded-full border-4 border-white overflow-hidden hover:scale-110 transition-all duration-300 cursor-pointer',
                    index === activeIndex && 'border-white z-10 grayscale-0',
                  )}
